...
const COUNTIES = [
  { value: "GWINNETT", label: "Gwinnett" }, { value: "DEKALB",   label: "DeKalb"   },
  { value: "COBB",     label: "Cobb"     }, { value: "FULTON",   label: "Fulton"   },
  { value: "CLAYTON",  label: "Clayton"  }, { value: "HENRY",    label: "Henry"    },
  { value: "MORROW",   label: "Morrow"   }, { value: "WALTON",   label: "Walton"   },
  { value: "ROCKDALE", label: "Rockdale" }, { value: "FORSYTH",  label: "Forsyth"  },
];
      ...
      {STEPS.map((step, i) => {
        ...
        return (
          <div key={step.id} className="ap-progress-step">
            ...
            <span className={`ap-step-label ${active?"active":""} ${done?"done":""}`}>{step.label}</span>
  ...
  return (
    <aside className="ap-reassurance">
      <div className="ap-reassurance-item">
        ...
        <div><strong>100% Confidential</strong><p>Your information is private and never shared without consent.</p></div>
      </div>
      <div className="ap-reassurance-item">
        <span className="ap-reassurance-icon"><PhoneIcon size={20} /></span>
        <div><strong>No Obligation</strong><p>A free assessment — zero commitment, zero pressure.</p></div>
      </div>
      <div className="ap-reassurance-item">
        <span className="ap-reassurance-icon">⚡</span>
        <div><strong>Quick Response</strong><p>A care coordinator will reach out within one business day.</p></div>
      </div>
    </aside>
  );
}
...
function Step1({ data, set, errors }) {
  return (
    <div className="ap-step-body">
      <p className="ap-step-intro">Tell us a little about yourself so we can reach you to set up the assessment.</p>
      <div className="ap-field-row">
        <div className="ap-field">
          <label htmlFor="firstName">First Name <span className="ap-req">*</span></label>
          <input id="firstName" type="text" placeholder="Jane" value={data.firstName}
        ...
        <div className="ap-field">
          ...
          <input id="lastName" type="text" placeholder="Smith" value={data.lastName}
      ...
      <div className="ap-field-row">
        <div className="ap-field">
          ...
          <input id="email" type="email" placeholder="jane@email.com" value={data.email}
        ...
        <div className="ap-field">
          ...
          <input id="phoneNumber" type="tel" placeholder="(770) 000-0000" value={data.phoneNumber}
...
function Step2({ data, set, errors }) {
  return (
    <div className="ap-step-body">
      <p className="ap-step-intro">Let us know your location and what type of care you are interested in.</p>

      <div className="ap-field-row">
        <div className="ap-field">
          <label htmlFor="county">County <span className="ap-req">*</span></label>
          <select id="county" value={data.county} onChange={e => set("county", e.target.value)}
        ...
        <div className="ap-field">
          ...
          <input id="city" type="text" placeholder="e.g. Lawrenceville" value={data.city}
      ...
      {/* Care type */}
      <div className="ap-field">
        <label>What type of care are you interested in? <span className="ap-req">*</span></label>
        <div className="ap-care-options">
          {CARE_OPTIONS.map(opt => (
            ...
            >
              ...
              {data.typeOfCare === opt.value && <span className="ap-care-check"><CheckIcon size={14} /></span>}
            </button>
          ))}
        </div>
      ...
      {/* Service sub-selector — only shown when HOME_CARE is selected */}
      {data.typeOfCare === "HOME_CARE" && (
        <div className="ap-field ap-service-field">
          <label>Which service are you interested in? <span className="ap-req">*</span></label>
          <div className="ap-service-options">
            {SERVICE_OPTIONS.map(opt => (
              <button key={opt.value} type="button"
                className={`ap-service-card${data.serviceType === opt.value ? " ap-service-card--active" : ""}`}
                onClick={() => set("serviceType", opt.value)}
                aria-pressed={data.serviceType === opt.value}
              >
                {opt.label}
                {data.serviceType === opt.value && <span className="ap-service-check"><CheckIcon size={11} /></span>}
              </button>
            ))}
          </div>
          {errors.serviceType && <span className="ap-err-msg">{errors.serviceType}</span>}
        </div>
      )}
      ...
      <div className="ap-summary">
        ...
        <div className="ap-summary-grid">
          ...
          {data.serviceType && (
            <div className="ap-summary-item">
              <span className="ap-summary-label">Service</span>
    ...
    <div className="ap-success">
      <div className="ap-success-icon" aria-hidden="true">
        <svg viewBox="0 0 56 56" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <path d="M18 28.5l7 7 13-13" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
      </div>
      <h2 className="ap-success-title">Thank You, {name}!</h2>
      <p className="ap-success-sub">Your assessment request has been received. A Helen's Home Care coordinator will contact you within <strong>one business day</strong>.</p>
      <div className="ap-success-next">
        <h3>What Happens Next</h3>
        <ol className="ap-success-steps">
          <li><span className="ap-success-step-num">1</span><div><strong>We review your request</strong><p>Our team reviews your information and assigns the right coordinator.</p></div></li>
          <li><span className="ap-success-step-num">2</span><div><strong>We call you</strong><p>A coordinator will reach out to you shortly to follow up.</p></div></li>
          <li><span className="ap-success-step-num">3</span><div><strong>Free in-home assessment</strong><p>Zero cost, zero obligation — we come to you.</p></div></li>
        </ol>
      </div>
...
export default function AssessmentPage() {
  ...
  const next = async () => {
    ...
    try {
      ...
      if (!res.ok) { const body = await res.json().catch(() => ({})); throw new Error(body.message || "Something went wrong. Please try again."); }
      setDone(true); scrollToCard();
    } catch (err) { setApiError(err.message); scrollToCard(); }
  ...
  return (
    <>
      <SEO
        title="Free Home Care Assessment | Helen's Home Care – Gwinnett County"
        description="Request a free in-home care assessment for your loved one. Helen's Home Care serves Gwinnett, DeKalb, Cobb, Fulton, and surrounding Atlanta-area counties."
      ...
      <div className="ap-page">
        <section className={`ap-hero ${heroIn ? "ap-hero--in" : ""}`}>
          ...
          <div className="ap-hero-inner">
            ...
            <h1 className="ap-hero-title">Request Your<br /><em>Free Assessment</em></h1>
            <p className="ap-hero-sub">Complete this short form and a Helen's Home Care coordinator will reach out within one business day.</p>
        ...
        <div className="ap-body">
          {done ? <SuccessState name={data.firstName} /> : (
              ...
              <div className="ap-form-col" ref={cardRef}>
                <div className="ap-card">
                  ...
                  <div className="ap-card-header">
                    ...
                    <h2 className="ap-card-title">{step === 1 ? "Your Contact Information" : "Your Location & Care Needs"}</h2>
                  </div>
                  {step === 1 && <Step1 data={data} set={set} errors={errors} />}
                  {step === 2 && <Step2 data={data} set={set} errors={errors} />}
                  {apiError && <div className="ap-api-error" role="alert"><WarningIcon size={16} style={{marginRight:6,verticalAlign:"middle"}} />{apiError}</div>}
                  <div className="ap-card-footer">
                    {step > 1 && <button type="button" className="ap-btn-back" onClick={back}>← Back</button>}
                    <button type="button" className={`ap-btn-next${step === STEPS.length ? " ap-btn-submit" : ""}`} onClick={next} disabled={loading}>
                      {loading ? "Submitting…" : step < STEPS.length ? <>Continue <span aria-hidden="true">→</span></> : <>Submit My Request ✓</>}
                    </button>
                  </div>
                </div>
              </div>
